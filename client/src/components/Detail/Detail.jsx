 ...
 useEffect(() => {
  ...
  .then(()=>setLoading(false))
   ...
   const detail = useSelector(state => state.detail);
    console.log({detail});
   const descripcionSinEtiq = detail.description?.replace(/a[^>]+>/g,"");
            ...
            <div className={style.arrayCont}>
              {
                 ...
                 ? <p className={style.platformCread}>{detail.platforms}</p>
                 :(detail.platforms && detail.platforms.map(({platforms}) =>(
                 <p key={'a' + z ++} value={platforms.name} className={style.arrays}>{platforms.name}</p>
                )))
              }
            </div>
              <h1 className={style.ArrayTittle}>Genres</h1>
              <span className={style.arrayCont}>
                ...
                <div>
                    ...
                    className={style.image} alt={`Img of ${detail.name} not found`}/>
